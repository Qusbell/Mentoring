...
public class WarningSystem : MonoBehaviour
{
    ...
    [Tooltip("감지할 큐브 레이어 (모든 큐브를 같은 레이어에 놓고 여기서 선택)")]
    public LayerMask detectionLayer = 1; // Default 레이어만 선택 (기본값)

    [Tooltip("경고 표시 높이 (큐브 위에서 얼마나 떠 있을지, 0.01 = 약간 위)")]
    ...
    {
        // 시작 시 중요 정보 출력
        Debug.Log("경고 시스템: 모든 큐브는 같은 레이어에 있어야 합니다. 선택된 레이어: "
            + LayerMask.LayerToName(GetSelectedLayer()));
    }
    ...
    private void CheckForCubeBelow()
    {
        ...
        if (Physics.Raycast(transform.position, Vector3.down, out hit, rayDistance, detectionLayer))
        {
            ...
            // 목적지 위치 계산 (아래 큐브의 윗면 + 이 큐브의 절반 높이)
